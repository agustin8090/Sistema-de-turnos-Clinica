...
namespace Datos
{
    public class medico_clinica
    {
        ...
        public bool Eliminar_medico(int legajo)
        {
            using (SqlConnection con = Conexion.ObtenerConexion())
            {
                string query = @"
        ...
        public List<medico> BuscarPorLegajoListar(int legajo)
        {
            List<medico> lista = new List<medico>();
        ...
        public List<medico> ListarPorEspecialidad(int idEspecialidad)
        {
            ...
            using (SqlConnection conexion = Conexion.ObtenerConexion())
            {
                ...
                while (reader.Read())
                {
                    ...
                    m.legajo = Convert.ToInt32(reader["Legajo"]); 
        ...
        public bool ExisteMedicoDuplicado(medico m)
        {
            using (SqlConnection conexion = Conexion.ObtenerConexion())
            {
                string consulta = @"
            ...
            WHERE Dni = @Dni
              ...
              AND Legajo <> @legajo"; //IGNORA EL MISMO MÉDICO
        ...
        public medico BuscarPorLegajo(int legajo)
        {
            ...
            using (SqlConnection conexion = Conexion.ObtenerConexion())
            {
                ...
                SELECT m.IdMedico, m.Legajo, m.DNI, m.Nombre, m.Apellido, m.Sexo, 
                       m.Nacionalidad, m.FechaNacimiento, m.Direccion,m.Email, m.Telefono,
        ...
        public bool validar_legajo(int legajo)
        {
            using (SqlConnection con = Conexion.ObtenerConexion())
            {
        ...
        public List<medico> BuscarPorDias(List<string> diasSeleccionados)
        {
            ...
            using (SqlConnection con = Conexion.ObtenerConexion())
            {
                // Base del query
                string query = @"
                    SELECT M.*, E.Nombre AS NombreEspecialidad
                    FROM Medico M
                    INNER JOIN Especialidad E ON M.IdEspecialidad = E.IdEspecialidad
                    WHERE 
                ";

                // Construcción dinámica -> (Dias LIKE '%Lunes%' OR Dias LIKE '%Martes%' ...)
                List<string> condiciones = new List<string>();

                for (int i = 0; i < diasSeleccionados.Count; i++)
                {
                    string paramName = "@dia" + i;
                    condiciones.Add($"M.DiasAtencion LIKE '%' + {paramName} + '%'");
                }
                ...
                for (int i = 0; i < diasSeleccionados.Count; i++)
                {
                    cmd.Parameters.AddWithValue("@dia" + i, diasSeleccionados[i]);
                }
                ...
                while (dr.Read())
                {
                    ...
                    m.nacionalidad = dr["Nacionalidad"].ToString();
                    m.fecha_nacimiento = (DateTime)dr["FechaNacimiento"];
                    m.direccion = dr["Direccion"].ToString();
                    m.id_localidad = (int)dr["IdLocalidad"];
                    m.id_especialidad = (int)dr["IdEspecialidad"];
                    m.nombre_especialidad = dr["NombreEspecialidad"].ToString();
                    m.telefono = dr["Telefono"].ToString();
                    m.email = dr["Email"].ToString();
                    m.dias_atencion = dr["DiasAtencion"].ToString();
                    m.hora_inicio = dr["HoraInicio"] == DBNull.Value ? null : (TimeSpan?)dr["HoraInicio"];
                    m.hora_fin = dr["HoraFin"] == DBNull.Value ? null : (TimeSpan?)dr["HoraFin"];
                    m.estado = (bool)dr["Estado"];
                    m.id_usuario = (int)dr["IdUsuario"];
